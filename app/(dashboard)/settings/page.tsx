    ...
    <div className="mx-auto max-w-4xl space-y-10 p-10 animate-fade-in">
            ...
            <div className="flex items-center gap-3 animate-fade-in-up">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-[#567C8D] to-[#567C8D] shadow-lg shadow-[#567C8D]/20">
                <Settings className="h-6 w-6 text-[#2F4156]" />
              </div>
              <div className="space-y-1">
                <h1 className="font-heading text-3xl md:text-[50px] font-bold tracking-tight leading-tight text-foreground">Settings</h1>
                <p className="text-muted-foreground text-base">Manage your account, notifications, and preferences.</p>
              </div>
            </div>
            ...
            {settingSections.map((section, sectionIndex) => (
              ...
              >
                <div className="mb-6 flex items-center gap-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#567C8D]/8">
                    <section.icon className="h-5 w-5 text-[#567C8D]" />
                  </div>
                  <div>
                ...
                <div className="space-y-2.5">
                  {section.items.map((item, itemIndex) => (
                    <button
                      key={item.label}
                      className="group flex w-full items-center justify-between rounded-xl border border-border bg-[var(--surface-page)] p-4 text-left transition-all duration-300 hover:border-[#567C8D]/30 hover:bg-card hover:shadow-sm"
                    >
                      <div className="flex items-center gap-3.5">
                        <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-[var(--muted)] transition-colors duration-300 group-hover:bg-[#567C8D]/8">
                          <item.icon className="h-[18px] w-[18px] text-muted-foreground transition-colors duration-300 group-hover:text-[#567C8D]" />
                        </div>
                        <div>
                      ...
                      {'toggle' in item && item.toggle ? (
                        <div className="h-7 w-12 rounded-full bg-[#567C8D] p-0.5 transition-colors shadow-inner">
                          <div className="h-6 w-6 translate-x-5 rounded-full bg-white shadow-md transition-transform" />
                        </div>
                      ) : (
                        <ChevronRight className="h-4 w-4 text-muted-foreground transition-all duration-300 group-hover:translate-x-1 group-hover:text-[#567C8D]" />
                      )}
                    </button>
                  ))}
                </div>
